				...
				<div class="col-lg-8 offset-lg-2">
					<div class="aplpg-title-area text-center wow fadeInUp">
						<div class="aplpg-headline">
							<h3>Liste des Types de payements</h3>
						</div>
					</div>
				</div>
			</div>
    ...
    <div class="container-fluid py-4" v-else>
      <div class="row">
        <div class="col-12">
          ...
          <span style="--i:1" class="opens" v-if="cathegorie.publish === 1">
            <i class="bi bi-power" @click="publish(cathegorie.id , cathegorie.publish)"></i>
          </span>
  ...
  methods: {
    async fetchActualites() {
        try {
          ...
          this.switchValue = response.data.data.reduce((acc, cathegorie) => {
        acc[cathegorie.id] = cathegorie.IsActive === 1;
        return acc;
      }, {});
          this.loading= false
    ...
    async confirmDeletedoc() {
      ...
      try {
        ...
        const response = await axios.delete(`/payment-types/${this.ToDeleteId}`, {
          headers: {
            Authorization: `Bearer ${this.loggedInUser.token}`,
...
console.log('dataMpme',dataMpme);
  ...
  async submit(){
    ...
    this.v$.step1.$touch();
      ...
      if (this.v$.$errors.length == 0) {
        ...
        try {
          const response = await axios.post("/payment-types", dataCath, {
            headers: {
              "Content-Type": "multipart/form-data",
              Authorization: `Bearer ${this.loggedInUser.token}`,
            },
 ...
 async updatedoc(id){
  ...
  this.loading = true
    ...
    try {
       const response = await axios.get(`/payment-types/detail/${id}`, {
         headers: {
...
console.log('dataCath',dataCath,this.ToDeleteId);
     ...
     try {
       const response = await axios.put(`/payment-types/${this.ToDeleteId}`,dataCath, {
         headers: {
