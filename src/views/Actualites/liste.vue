...
<Loading v-if="loading" style="z-index: 99999;"></Loading>
  ...
  <button class="search-btn"   ><i class="bi bi-search"></i></button>
        ...
        <div class="icon"  @click="$router.push({ path: '/actualites/ajouter' })" >
    ...
    <section class="aplpg-blog-section">
      ...
      <span> Vous n'avez pas encore d'actualité, vous pouvez également en ajouter une !! </span>
    </div>
          ...
          <span style="--i:1"  class="dow">
            <i class="bi bi-eye"  @click="$router.push({ path:  '/actualites/detail/'+ actualite.id })"></i>
           
          </span>
          <span style="--i:1" class="update">
            <i class="bi bi-pen" @click="$router.push({ path:  '/actualites/update/'+ actualite.id })"></i>
...
paginatedItems() {
  ...
  methods: {
    async fetchActualites() {
        try {
          const response = await axios.get('/actualites/sans-pagination', {
          headers: {
    ...
    async confirmDeletedoc() {
      ...
      try {
        ...
        const response = await axios.delete(`/actualites/${this.ToDeleteId}`, {
          headers: {
            Authorization: `Bearer ${this.loggedInUser.token}`,
